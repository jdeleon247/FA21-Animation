...
inline a3_SpatialPose* a3spatialPoseOpIdentity(a3_SpatialPose* pose_out, a3_SpatialPose const* pose_ctrl[1], a3f64 const* param[1])
{
	...
	pose_out->rotate = a3vec4_w;
	pose_out->scale = a3vec4_one;
	pose_out->translate = a3vec4_w;
...
inline a3_SpatialPose* a3spatialPoseOpConstruct(a3_SpatialPose* pose_out, a3vec4 rotation, a3vec4 scale, a3vec4 translate)
{
	pose_out->rotate = rotation;
...
inline a3_SpatialPose* a3spatialPoseOpCopy(a3_SpatialPose* pose_out, a3_SpatialPose const* pose_ctrl[1], a3f64 const* param[1])
{
	...
	pose_out->rotateQuat = pose_ctrl[0]->rotateQuat;
	pose_out->scale = pose_ctrl[0]->scale;
	pose_out->translate = pose_ctrl[0]->translate;
...
inline a3_SpatialPose* a3spatialPoseOpInvert(a3_SpatialPose* pose_out, a3_SpatialPose const* pose_ctrl[1], a3f64 const* param[1])
{
	...
	a3real4GetNegative(pose_out->translate.v, pose_ctrl[0]->translate.v);  // Additive -> make negative
...
inline a3_SpatialPose* a3spatialPoseOpSplit(a3_SpatialPose* pose_out, a3_SpatialPose const* pose_ctrl[2], a3f64 const* param[1])
{
	...
	pose_out->rotate.z = a3trigValid_sind(pose_ctrl[0]->rotate.z - pose_ctrl[1]->rotate.z);
	...
	pose_out->translate.x = pose_ctrl[0]->translate.x - pose_ctrl[1]->translate.x;
	pose_out->translate.y = pose_ctrl[0]->translate.y - pose_ctrl[1]->translate.y;
	pose_out->translate.z = pose_ctrl[0]->translate.z - pose_ctrl[1]->translate.z;
...
{
	a3_SpatialPose const result;// = { a3mat4_identity, *a3vec4_w.v, a3vec4_w, a3vec4_one, a3vec4_w /*, ...*/ };
	return result;
}
...
// Equivalent to a constructor, this operation returns/sets a pose constructed using the components provided.
inline a3_HierarchyPose* a3hierarchyPoseOpConstruct(a3_HierarchyPose* pose_out, a3vec4 rotation, a3vec4 scale, a3vec4 translate, a3ui32 numNodes)
{
	for (a3index i = 0; i < numNodes; ++i)
	{
		a3spatialPoseOpConstruct(pose_out->pose + i, rotation, scale, translate);
	}
	return pose_out;
}
...
inline a3_HierarchyPose* a3hierarchyPoseOpFK(a3_HierarchyPose* pose_out, a3_Hierarchy const* hierarchy, a3_HierarchyPose const* poseCtrl[2])
{
	...
	for (; itr < end; ++itr)
	{
		if (itr->parentIndex >= 0)
			a3real4x4Product(poseCtrl[0]->pose[itr->index].transformMat.m,
				poseCtrl[0]->pose[itr->parentIndex].transformMat.m,
				poseCtrl[1]->pose[itr->index].transformMat.m);
		else
...
inline a3_HierarchyPose* a3hierarchyPoseOpIK(a3_HierarchyPose* pose_out, a3_Hierarchy const* hierarchy, a3_HierarchyPose const* poseCtrl[2])
{
	...
	{
		if (itr->parentIndex >= 0)
			a3real4x4Product(poseCtrl[1]->pose[itr->index].transformMat.m,
				pose_out->pose[itr->parentIndex].transformMat.m,
				poseCtrl[0]->pose[itr->index].transformMat.m);
		else
